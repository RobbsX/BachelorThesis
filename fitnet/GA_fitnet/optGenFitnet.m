function [l1,l2,activation,optimizer] = optGenFitnet(Data,datasz)
...
Popsz = 30; % size of population % use 40
...
rng = [1,0,1,1; ... % setting the range of each parameter
    25,25,4,8]; % row1: lowest value, row2: highest value
%emaBitsz = adaptBitsz(max_length); % Adapt Bit size of EMAs
bit_size = [5,5,2,3]; % setting the size of the bits
        

P = GgeneratePopFitnet(Popsz, bit_size, rng, datasz);
[P, P_dec] = GfitnessFitnet(Data, P, bit_size, rng);

% evolving the population
if sum([P{:,2}]) ~= 0
    for i = 1:15
        P = GrecombineFitnet(P, pCross, one_or_two_point, bit_size, rng, datasz);
        P = GmutateFitnet(P, pMutate, bit_size, rng, datasz);
        [P, P_dec] = GfitnessFitnet(Data, P, bit_size, rng);
