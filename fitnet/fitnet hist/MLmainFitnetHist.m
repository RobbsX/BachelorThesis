...
%% Prepare / Load struct
%MScollectionFitnetHist = struct; % just used for the first time
load('MScollectionFitnetHist.mat') 
if useModel == 0
    istep = length(fieldnames(MScollectionFitnetHist))+1:N;
else
    istep = 1:length(fieldnames(MScollectionFitnetHist));
end
...
if useModel == 0
    ...
    currentMS = "ModelSetFit" + i;
    ModelSet = MScollectionFitnetHist.(currentMS); 
end
...
for w = 1:height(wfaPeriod)
    ...
    if w ~= 0 && useModel == 0 % use to test whole code quickly or to save and continue process
        while check == 0
            % Train model            
            params = {14, 0, 'relu', 'trainbr'};
            [net, validationRMSE] = MLmodelsFitnet(X_IS, D_IS, params);
...
ModelSetTable.Properties.VariableNames = {'Model','RSME','RSQ','Layer Sizes',...
    'Activation','Optimizer','OS Std','OS Profit'};
...
plotting = 1;
   if plotting == 1
       ...
       figEC_OS = figure;
       plot(EClist, 'c-'); % OS curve
       hold on;
       plot(Close, 'b-'); % IS curve 
       if MaxDD ~= 0
       ...
       for l = 1:size(cell2mat(ModelSet(:,2)),1)
           if length(net.layers)-1 == 1
               layers(l,1:2) = [ModelSet{l,1}.layers{1}.size 0];
           else
               layers(l,1:2) = [ModelSet{l,1}.layers{1}.size ModelSet{l,1}.layers{2}.size];
           end
       end
   ...
   saveName = "ModelSetFit" + i;
   MScollectionFitnetHist.(saveName) = ModelSet;
...
% Compare fitnet thumb rule and opt
% compare = figure;
% plot(Close,'Color','#74356E','LineWidth',2); % benchmark
% hold on;
% plot(ECfitnetOpt,'Color','#2D42C2','LineWidth',2); % fitnet opt
% plot(ECfitnetTR,'Color','#2F4858','LineWidth',2); % fitnet thumb rule hist
% hold off;
% 
% title("Fitrnet " + filename);
% yline(0,'k-');
% xlabel("Time");
% ylabel("Profit (absolut)");
% legend('benchmark','EC_s_m_a_l_l','
...
%% Not used
% Plots of the results of ModelSet %%
% 
% % Add zero to layer if only one hidden layer 
% layers = zeros(size(ModelSet(:,1),1),2);
% for l = 1:25 %1:size(ModelSet(:,1),1)
%     if length(net.layers)-1 == 1
%         layers(l,1:2) = [ModelSet{l,1}.layers{1}.size 0];
%     else
%         layers(l,1:2) = [ModelSet{l,1}.layers{1}.size ModelSet{l,1}.layers{2}.size];
...
% if plotting == 2 % X-Axis labled as String would be better
%     figure;histogram(idxac, 4,'FaceColor','b'); 
%     title("Activation Historgram");
% end
% 
% % Optimizer Histogram - only if ModelSet is complete with nets
% % optimizer = ModelSet(:,6); optimizer = cell2mat(optimizer);
% % [~, ~, idxlambda] = unique(optimizer(:,1));
% % if plotting == 1 % X-Axis labled as String would be better
% %     figure;histogram(optimizer, 20,'FaceColor','r'); 
% %     title("Optimizer Historgram");
% % end
% 
% 
