function [EC,endvalueEC2,tradesAll,tradesAllClean] = evalTradesOffestTSLTPstd(data,position,SLTPparams,predictOffset,xnodes,RiskManagement,plotparam)
% This function evaluates the Trades. The SL and TP is calculated using the
% std of the last sPeriod days and a factor. Additionally, the SL is
% adjusted after every openPrice if the new SL is lower than before (for
% long trades).
% Used by MLmainFitnetTSLTP.m
...
for ii = 1:length(position)
    ...
    if position(ii) == 1
        ...
        % Calculate SL per tradining day
        tradeSL = zeros(predictOffset,1);
        SL = tradeOpen*(1-xSL); % eg. 1,018
        tradeSL(1,1) = SL;
        for ll = 2:length(dataTrade)
            newSL = dataTrade(ll,1)*(1-xSL);
            if newSL > SL
                SL = newSL;
            end
            tradeSL(ll,1) = SL;
        end
        %tradeSL = dataTrade(:,1).*(1-xSL);
        tradeTP = dataTrade(:,1).*(1+xTP);
        ...
        while index <= LT
            ...
            elseif and(tmpLow <= tradeSL(index), tmpHigh >= tradeTP(index))
